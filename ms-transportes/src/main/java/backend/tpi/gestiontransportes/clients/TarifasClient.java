...
public class TarifasClient {
    ...
    public TarifasClient(@Value("${ms.solicitudes.url}") String baseUrl, RestClient.Builder restClientBuilder) {
        ...
        return this.restClient.get()
                ...
                .onStatus(status -> status.value() == HttpStatus.NOT_FOUND.value(), (request, response) -> {
                    ...
                    throw new ResponseStatusException(HttpStatus.NOT_FOUND,
                            "la tarifa con ID " + id + " no fue encontrada.");
    ...
    public TarifaDTO obtenerTarifaParaVolumen(BigDecimal volumen) {
        ...
        List<TarifaDTO> tarifasEncontradas = this.restClient.get()
                ...
                .onStatus(status -> status.value() == HttpStatus.NO_CONTENT.value(), (req, res) -> {
                    // Si el servicio responde 204, significa que no hay tarifas, lo tratamos como
                    // NOT_FOUND
                    throw new ResponseStatusException(HttpStatus.NOT_FOUND,
                            "No se encontró ninguna tarifa para el volumen: " + volumen);
                })
        ...
        if (tarifasEncontradas == null || tarifasEncontradas.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND,
                    "No se encontró ninguna tarifa válida para el volumen: " + volumen);
        }
